...
class BreachService:
    ...
    def _load_bloom_filter(self):
        """Load the Bloom Filter from disk or regenerate from MongoDB."""
        import os
        from pathlib import Path
        logger = logging.getLogger(__name__)
        try:
            from pybloom_live import BloomFilter
            # 1. Try loading from pre-generated file (fastest)
            bloom_path = Path(__file__).parent.parent.parent / "seed" / "breaches.bloom"
            if bloom_path.exists():
                with open(bloom_path, "rb") as fh:
                    logger.info("Loaded Bloom Filter from file signature.")
                    return BloomFilter.fromfile(fh)

            # 2. Self-healing: Build in memory from MongoDB if file is missing (Safe for Cloud)
            logger.info("Bloom signature file missing. Initiating self-healing regeneration...")

            # Estimate capacity (standard padding for growth)
            total_docs = self.col.count_documents({})
            capacity = max(10000, total_docs * 200) # Assuming avg 200 accounts per breach
            f = BloomFilter(capacity=capacity, error_rate=0.001)

            # Efficiently stream all exposed email addresses
            cursor = self.col.find({}, {"affected_accounts.email": 1})
            count = 0
            for doc in cursor:
                for acc in doc.get("affected_accounts", []):
                    email = acc.get("email")
                    if email:
                        f.add(email.lower())
                        count += 1

            logger.info(f"Self-healing complete. Bloom Filter rebuilt with {count} signatures.")
            return f

        except Exception as exc:
            # Fallback if pybloom_live is not installed or DB is unreachable
            logger.warning("Bloom filter system inactive (Local/DB dependency failure): %s", exc)
        return None
