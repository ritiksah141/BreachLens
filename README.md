...
[![Backend Tests](https://img.shields.io/badge/Backend_Tests-634%20passing-brightgreen.svg)](backend/tests/)
[![Frontend Tests](https://img.shields.io/badge/Frontend_Tests-255%20passing-brightgreen.svg)](frontend/src/app/)
[![Coverage](https://img.shields.io/badge/Coverage-84%25-brightgreen.svg)](backend/htmlcov/)
...
BreachLens is a production-ready cyber threat intelligence platform designed to track and analyze data breaches globally. The platform provides:

- **Scalable Exposure Checks**: High-speed email exposure detection using an in-memory Bloom Filter architecture.
- **Personal Defense Hub**: A dedicated workspace for authenticated users featuring Risk Genome Breakdown (DNA deconstruction of leaks) and Tactical Defense Playbooks.
...
### System Architecture
Detailed technical documentation on the system integration, data flow, and infrastructure components can be found in the [ARCHITECTURE.md](ARCHITECTURE.md) file.
...
- **Automated OSINT Sync** - GitHub Action-powered daily synchronization with the global HIBP repository.
- **Dual-Tier CI/CD** - Automated quality gates for both Backend and Frontend via GitHub Actions.
...
- **Defense Hub** - Personal security posture center with Threat Magnitude (0-10) benchmarking.
- **Risk Genome** - Categorization of exposed data into four functional vectors: Identity, Credentials, Financial, and Technical.
- **Tactical Playbook** - Dynamic, priority-ranked remediation plans (Critical, High, Medium actions).
- **Geospatial Mapping** - Leaflet-js tactical map with GeoJSON telemetry.
- **Interactive Alerts** - Real-time "Open Alerts" overlay for monitoring unacknowledged security events.
- **Dark/Light Themes** - System-aware theming with CSS custom properties.
- **Comprehensive Testing** - 889 tests across backend (634) and frontend (255).
...
- **Interactive Breach Map**: Leaflet.js with GeoJSON markers and severity coloring.
...
The project has comprehensive test coverage: 889 total tests.
